...
function renderCheerMode(){const node=$('cheer-mode');if(!node)return;const state=cheerMode==='cloud'?'is-cloud':cheerMode==='connecting'?'is-connecting':'is-local';node.textContent=cheerMode==='cloud'?'● 即時共用':cheerMode==='connecting'?'● 應援連線中':'● 本機模式';node.className=`cheer-status ${state}`;}
