...
function renderHrm(){const p=people.find(x=>x.team==='HRM'),progress=personProgress(p),pct=rate(progress,p.target);$('hrm-bar').style.width=`${hasLoaded?Math.min(pct,100):0}%`;$('hrm-badge').textContent=hasLoaded?`${pct.toFixed(1)}%`:'—';$('hrm-progress').textContent=hasLoaded?`${money(progress)} / ${money(p.target)}${pct>=100?'・挑戰成功！':''}`:'登入後同步季進度（含在途）';}
...
function renderRewards(){
  renderTeamRewardRanking('monthly-team-ranking',rankedTeamProgress(personMonthlyProgress,team=>stats(team.id).target),{ready:hasLoaded&&hasMonthlyProgress,emptyMessage:hasLoaded?'尚未取得 AP 月進度，請完成資料庫升級後重新上傳。':'登入同步後顯示 AP 欄每月團隊排名。',targetLabel:'開門紅責任目標'});
  renderTeamRewardRanking('champion-team-ranking',rankedTeamProgress(personProgress,team=>stats(team.id).target),{ready:hasLoaded,emptyMessage:'登入同步後顯示季進度總冠軍排名。',targetLabel:'開門紅目標'});
  const node=$('personal-milestone-ranking');if(!hasLoaded){node.innerHTML='<p class="reward-empty">登入同步後顯示 26 位人員的 3K／4K／5K 即時進度。</p>';return;}
...
function setBusy(value){busy=value;$('dialog-sync-button').disabled=value;$('raw-file').disabled=value||!canWrite;$('monthly-file').disabled=value||!canWrite;$('performance-file').disabled=value||!canWrite;$('raw-file-label').classList.toggle('is-disabled',value||!canWrite);$('monthly-file-label').classList.toggle('is-disabled',value||!canWrite);$('performance-file-label').classList.toggle('is-disabled',value||!canWrite);$('refresh-button').disabled=value||!currentUser;$('manage-button').disabled=value||!canWrite;}
...
async function fetchPerformanceRecords(includeMonthly=true){const columns=`branch, advisor_name, quarter_target, quarter_progress, ${includeMonthly?'monthly_progress, ':''}quarter_rate, fund_progress, insurance_progress, source_date`;return supabase.from('performance_records').select(columns).neq('advisor_name',branchTargetRecordName).order('branch').order('advisor_name');}
async function loadPerformance({announce=true}={}){if(!supabase||!currentUser||busy)return;setBusy(true);if(announce)setStatus('裁判正在同步最新戰況…');try{let{data,error}=await fetchPerformanceRecords(true);hasMonthlyProgress=!error;if(error&&String(error.message||'').includes('monthly_progress'))({data,error}=await fetchPerformanceRecords(false));if(error)throw error;performance=recordMap(data||[]);hasLoaded=true;const dates=sourceDates(),monthlyNote=hasMonthlyProgress?'AP 月進度已同步。':'尚未建立 AP 月進度欄位。';setStatus(`同步完成！共 ${Object.keys(performance).length} 位選手。${dates.length?`資料日期：${dates.join('、')}。`:''}${monthlyNote}`,'success');setUpload(`戰況同步完成，共 ${Object.keys(performance).length} 筆。${monthlyNote}`,'success');render();}catch(error){setStatus(`同步失敗：${error.message||'請稍後再試。'}`,'error');setUpload(`同步失敗：${error.message||'請稍後再試。'}`,'error');}finally{setBusy(false);}}
async function applySession(session){currentUser=session?.user||null;canWrite=false;hasLoaded=false;hasMonthlyProgress=false;performance={};$('login-button').hidden=Boolean(currentUser);$('manage-button').hidden=true;$('signout-button').hidden=!currentUser;if(!currentUser){setCloud('尚未登入');setStatus('登入後即可同步比賽進度；隊伍與目標可先行查看。');setBusy(false);render();return;}try{setCloud('裁判驗證中…');const{data:role,error}=await supabase.rpc('my_performance_role');if(error)throw error;if(!role){setCloud('帳號未授權','error');setStatus('此帳號尚未取得查看戰況的權限。','error');setBusy(false);render();return;}canWrite=['admin','editor'].includes(role);$('manage-button').hidden=!canWrite;setCloud(role==='viewer'?'已登入・觀賽':'雲端已連線','success');setBusy(false);await loadPerformance({announce:false});}catch(error){setCloud('驗證失敗','error');setStatus(`無法驗證權限：${error.message||'請稍後再試。'}`,'error');setBusy(false);render();}}
...
async function init(){cheerRecords=readLocalCheers();render();setupEvents();$('manager-login-form').hidden=!hasManagerUploadAccount;$('login-divider').hidden=!hasManagerUploadAccount;$('manager-email').value=hasManagerUploadAccount?String(config.uploadAccountEmail):'';setBusy(false);setInterval(()=>void refreshCheerDay(),30000);if(!isConfigured){cheerMode='local';renderCheerMode();$('login-button').disabled=true;setCloud('尚未設定雲端','error');setStatus('請先完成 Supabase 連線設定。','error');return;}try{const{createClient}=await import('https://esm.sh/@supabase/supabase-js@2');supabase=createClient(config.url,config.anonKey);await loadCheers();supabase.auth.onAuthStateChange((_event,session)=>void applySession(session));const{data:{session}}=await supabase.auth.getSession();await applySession(session);}catch(error){cheerMode='local';cheerRecords=readLocalCheers();renderRace();renderCheerMode();$('login-button').disabled=true;setCloud('雲端連線失敗','error');setStatus(`無法連接績效系統：${error.message||'請檢查網路。'}`,'error');}}
void init();
